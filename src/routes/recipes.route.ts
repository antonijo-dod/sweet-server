...
class RecipesRoute implements Routes {
    ...
    private initializeRoutes() {
        ...
        this.router.put(
            `${this.path}/:id(\\d+)`,
            adminAuthMiddleware,
            validationMiddleware(CreateRecipeDto, 'body', true),
            this.recipesController.updateRecipe,
        );
        this.router.delete(`${this.path}/:id(\\d+)`, adminAuthMiddleware, this.recipesController.deleteRecipe);
        this.router.put(`${this.path}/:id(\\d+)/status`, adminAuthMiddleware, this.recipesController.updateStatusRecipe);
    }
}
